...
def adaboost_original(n_learners, epochs_lst, batch_size, sample_ratio=3, filename="temp.txt", file_prefix=""):
    ''' adaboost of multi classification (original alg)'''
    num_classes = 10
    K = float(num_classes)
    ...
    for i in range(n_learners):
        ...
        model, history = train(x_train_i, y_train_i, x_test, y_test, model, batch_size, epochs, data_augmentation=True, name_prefix="adaboost-model-"+str(i))
...
def bagging_train_model(n_learners, epochs_lst, batch_size, votefuns, filename="temp.txt", file_prefix="", random=True):
    ...
    for i in range(n_learners):
        ...
        if random:
            ...
            x_train_i = x_train
            y_train_i = y_train
        model, history = train(x_train_i, y_train_i, x_test, y_test, model, batch_size, epochs, data_augmentation=True, name_prefix="bagging-model-"+str(i))
...
def stack_train_model_super(n_learners, epochs_lst, batch_size, meta_epochs=40, filename="temp.txt"):
    ...
    for i in range(n_learners):
        epochs = epochs_lst[i]
        model = build_model(x_train, num_classes)
        model, history = train(x_train, y_train, x_test, y_test, model, batch_size, epochs, data_augmentation=True, name_prefix="super-model-"+str(i))
        print("model %d finished" % (i))
        scores = model.evaluate(x_test, y_test, verbose=1)
        test_accuracy_records.append(scores[1])
...
def stack_train_model(n_learners, epochs_lst, batch_size, meta_epochs=40, filename="temp.txt"):
    ...
    for i in range(n_learners):
        epochs = epochs_lst[i]
        model = build_model(x_train, num_classes)
        model, history = train(x_train, y_train, x_test, y_test, model, batch_size, epochs, data_augmentation=True, name_prefix="stack-model-"+str(i))
        print("model %d finished" % (i))
        scores = model.evaluate(x_test, y_test, verbose=1)
        test_accuracy_records.append(scores[1])
...
def stack_loading_model_super(saved_model_files, meta_epochs=40, filename="temp.txt"):
    ...
    for i in range(n_learners):
        ...
        meta_x_test[:, i*num_classes:i*num_classes + num_classes] = models[i].predict(x_test, verbose=0)
    # meta_y_train = y_train # use one hot encode
    meta_y_train = y_val # use one hot encode
    meta_y_test = y_test
    super_model = meta_model(n_learners, num_classes)
    # callbacks
    save_dir = os.path.join(os.getcwd(), 'super_models')
...
def test1():
    ...
    n_learners = len(saved_model_files)
    bagging_loading_model(n_learners, saved_model_files, votefuns, "cnn-bagging.txt", file_prefix="", random=True)
...
def adaboost_original_test():
    n_learners = 3
    epochs_lst = [40, 40, 40]
    batch_size = 32
    sample_ratio = 3
    adaboost_original(n_learners, epochs_lst, batch_size, sample_ratio, "cnn-adaboost-original.txt")

def super_test():
    n_learners = 3
    epochs_lst = [100, 100, 100]
    batch_size = 32
    meta_epochs = 200
    stack_train_model_super(n_learners, epochs_lst, batch_size, meta_epochs, filename="cnn-super.txt")
...
def bagging_test(random):
    n_learners = 3
    batch_size = 32
    epochs_lst = [100, 100, 100]
    votefuns = [weighted_vote,  majority_vote]
    bagging_train_model(n_learners, epochs_lst, batch_size, votefuns, "cnn-bagging.txt", file_prefix="", random=random)
...
if __name__ == "__main__":
    print("Hello UW!")
    if len(sys.argv) != 2:
        print("Usage: python cnn_ensemble.py test_type")
        ...
        print("test type = 7: adaboost original")
        sys.exit()
    ...
    if test_type == 1:
        ...
        bagging_test(random=True)
    elif test_type == 4:
        bagging_test(random=False)
    elif test_type == 5:
        snapshot_test()
    elif test_type == 6:
        super_test()
    elif test_type == 7:
        adaboost_original_test()
    else:
...
'''
    ...
    # saved_model_files = ['saved_models/keras_cifar10_trained_model_4.h5', 'saved_models/keras_cifar10_trained_model_6.h5']
    saved_model_files = ['adaboost.0.final.hdf5', 'adaboost.1.final.hdf5', 'adaboost.2.final.hdf5']
    meta_epochs = 20
    # stack_loading_model(saved_model_files, meta_epochs, filename="cnn-stack.txt")
    stack_loading_model_super(saved_model_files, meta_epochs, filename="cnn-super.txt")
'''
